...
**Status:** Draft v0.1, revised 2026-09-07, maintained by The Service Marketing Guys.
...
- **MCP server card** at `/.well-known/mcp/server-card.json`. States the MCP transport and tool names. A Level 2 business MUST publish a server card whose tool list includes the OSP tool names it implements. OSP requires the minimal shape given after this list.
...
**Server card minimal shape.** OSP requires these fields and permits any additional fields the card format defines:
...
|---|---|---|
| `on_site` | Work is performed at the customer's address. | `coverage` is REQUIRED. `bookingPolicy.requiresAddress` defaults to `true`. `customer.vehicle` is not accepted: a server MUST ignore it and MUST NOT return an error because it was sent. |
...
| `vehicleDetailsAccepted` | boolean | OPTIONAL, default false | Allowed only when `business.serviceModel` is `both` (the manifest schema rejects it otherwise): whether `customer.vehicle` is accepted in booking requests. `in_shop` businesses accept it regardless; `on_site` businesses ignore it. |
...
5. Otherwise `covered` is `false` and `matchedBy` is `none`. A server that does not evaluate radius falls through to this step even when radius coverage is published.
...
- `matchedBy` is REQUIRED and is one of `postalCodes`, `location`, `radius`, `in_shop`, `none`.
...
|---|---|---|
| `policy` | Computed only from the manifest: hours, exceptions, `minimumDaysAhead`, `maximumDaysAhead`, `weekendRequests`, and which windows overlap the published open hours for the date. Windows without published `start` and `end` use the default boundaries in section 5.7.1. | Every Level 3 server MUST support this mode. |
...
| `booking_request_failed` | downstream failure after the input was accepted; message is safe to relay |
...
The reference implementation runs on the maintainer's client websites. As of 2026-09-07 it implements Level 1 (manifest and schema.org markup), Level 2 (the four read tools over MCP; `check_availability` is absent, see D2), and the booking half of Level 3. It diverges from this draft as follows. Items marked implemented are recorded so the history of the draft stays legible; the rest are build tasks, not specification changes.
...
| D11 | `check_coverage` evaluation order, `matchedBy`, and `message` | Defined, with `postalCodes`, `location`, `radius`, `in_shop`, `none` | Output has `covered`, `areaName`, `areaUrl` or `phone`. No `matchedBy`. Radius coverage is not evaluated (would fall through to `none`); coverage is exact ZIP lists from service-area rows and shop rows. `in_shop` tenants always publish ZIP lists, so the no-coverage branch is never reached. |
